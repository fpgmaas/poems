...
def get_last_word(line):
    ...
    line = line.strip()
    return (
        re.findall(r"\s([^\.?!,\s]+)[\.?!,\s']*$", line)[0]
        if re.findall(r"\s([^\.?!,\s]+)[\.?!,\s']*$", line)
        else None
    )
...
def rhymes_all(word):
    ...
    if len(phones) > 0:
        return [
            w
            for phone in phones
            for w in pronouncing.rhyme_lookup.get(pronouncing.rhyming_part(phone), [])
            if w != word
        ]
    else:
        return []
        ...
        if rhyme_scheme[i] == "":
            if last_words_per_line[i] is not None:
                ...
                if np.any([x in rhyme_list for x in last_words_per_line]):
                    rhyme_scheme[
                        (np.array([x in rhyme_list for x in last_words_per_line]) & (rhyme_scheme == ""))
                    ] = alphabet[k % 26]
                k += 1
            else:
                rhyme_scheme[i] = "?"
